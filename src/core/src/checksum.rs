...
impl StreamingHasher {
    pub fn new(algorithm: ChecksumAlgorithm) -> Self {
        let hasher: Box<dyn HasherImpl> = match algorithm {
            ...
            ChecksumAlgorithm::Xxh3 => Box::new(Xxh3Hasher {
                hasher: Xxh3::new(),
            }),
        };
...
mod tests {
    ...
    async fn test_blake3_hash() {
        ...
        let hash = compute_archive_checksum(&file, ChecksumAlgorithm::Blake3)
            .await
            .unwrap();
    ...
    async fn test_sha256_hash() {
        ...
        let hash = compute_archive_checksum(&file, ChecksumAlgorithm::Sha256)
            .await
            .unwrap();
        assert!(!hash.is_empty());
    ...
    async fn test_verify_match() {
        ...
        let hash = compute_archive_checksum(&file, ChecksumAlgorithm::Blake3)
            .await
            .unwrap();
        let ok = verify_checksum(&file, &hash, ChecksumAlgorithm::Blake3)
            .await
            .unwrap();
        assert!(ok);
    }
    ...
    async fn test_verify_mismatch() {
        ...
        let ok = verify_checksum(&file, "wrongchecksum", ChecksumAlgorithm::Blake3)
            .await
            .unwrap();
        assert!(!ok);
    }
